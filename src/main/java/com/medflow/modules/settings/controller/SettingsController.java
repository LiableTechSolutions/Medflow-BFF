...
class SettingsController {
  ...
  @GetMapping
  @Operation(summary = "List settings", description = "All workspace settings, sorted by key.")
  ApiResponse<List<SettingResponse>> getAll() {
    return ApiResponse.success("Settings retrieved successfully", service.getAll());
  }
  ...
  ApiResponse<SettingResponse> get(@PathVariable @Pattern(regexp = KEY_PATTERN) String key) {
    return ApiResponse.success("Setting retrieved successfully", service.get(key));
  }

  @PutMapping("/{key}")
  @PreAuthorize("hasRole('ADMIN')")
  @Operation(summary = "Upsert setting", description = "Admin-only: creates or replaces a setting value.")
  ApiResponse<SettingResponse> put(@PathVariable @Pattern(regexp = KEY_PATTERN) String key,
      @Valid @RequestBody UpdateSettingRequest request) {
    return ApiResponse.success("Setting saved successfully", service.put(key, request));
  }
}
